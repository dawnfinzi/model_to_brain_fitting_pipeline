...
from utils.fit_utils import get_indices, mapping
...
def main(
    ...
    ROIS = ROI_NAMES[1:]  # all ROIs except 'Unknown'
    ...
    mgh_file = mgh.load(
        FS_PATH + "subj" + target_subj + "/" + hemi + "." + target_roi + ".mgz"
    )
    ...
    del sorted_betas, stream_betas, betas
    ...
    if pooled:
        ...
        if n_source_voxels is None:
            ...
            for l in ROIS:
                if subsamp_type == 0:  # select a random subset of voxels per ROI
                    ...
                    log(f"ROI: {l}, Percentage of voxels used: {n_source_voxels/n}")
        ...
        for sidx, s in enumerate(source_subj):
            by_source_rsquared_array[s] = mapping(
                stacked_vox[sidx],
                target_sorted_validation_betas,
                all_splits,
                ROIS,
    ...
    h5f = h5py.File(
        ...
        + target_roi
        + (
            "_Ridge" if mapping_func == "Ridge" else ""
        )  # default is PLS, not included in filename for backwards compat
        + (
            "_1CV" if CV == 1 else ""
        )  # default is not to CV, not included in filename for backwards compat
        + "_"
        + str(num_splits)
        + "splits_"
        + "_subsample_"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mapping_func", type=str, default="PLS")  # PLS or Ridge
    parser.add_argument(
        "--CV", type=int, default=0
    )  # Cross-validate hyperparams (1) or not (0)
    parser.add_argument(
