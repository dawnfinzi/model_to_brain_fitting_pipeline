...
for h = 1:length(hemis)
    for l = 1:length(layers)
        fits(h).(layers{l})= h5read(fullfile(fits_path,[subjid, '_', hemis{h},'_', roi_name, '_alexnet_25c_fullfeats_fits.hdf5']), ['/',layers{l}]);
    end
end
...
if subj2subj == 1
    ...
    for h = 1:length(hemis)
        for s = 1:length(subj_keys)
            subj_fits(h).(['s' subj_keys{s}])= h5read(fullfile(fits_path,['subj2subj/', subjid, '_', hemis{h},'_streams_shrink10_ventral_othersubjs_fits.hdf5']), ['/',subj_keys{s}]);
        end
    end

    v_all = [subj_fits.s01 subj_fits.s05 subj_fits.s07];

    v_all_m = mean(v_all, 2);

    %% load subject to subject fits
    subj_fits = struct([]);
    for h = 1:length(hemis)
        for s = 1:length(subj_keys)
            subj_fits(h).(['s' subj_keys{s}])= h5read(fullfile(fits_path,['subj2subj/', subjid, '_', hemis{h},'_streams_shrink10_lateral_othersubjs_fits.hdf5']), ['/',subj_keys{s}]);
        end
    end

    l_all = [subj_fits.s01 subj_fits.s05 subj_fits.s07];

    l_all_m = mean(l_all, 2);

    %% load subject to subject fits
    subj_fits = struct([]);
    for h = 1:length(hemis)
        for s = 1:length(subj_keys)
            subj_fits(h).(['s' subj_keys{s}])= h5read(fullfile(fits_path,['subj2subj/', subjid, '_', hemis{h},'_streams_shrink10_parietal_othersubjs_fits.hdf5']), ['/',subj_keys{s}]);
        end
    end

    p_all = [subj_fits.s01 subj_fits.s05 subj_fits.s07];

    p_all_m = mean(p_all, 2);
end
...
if subj2subj == 1
    ...
    figure(f(1));
    niceSave(results_path,sprintf('/%s_ModelFitsvsSubjFits_match_%s', subjid, roi_name),[],[],{'png' 'svg'});
...
for r = 1:length(ROIs)
    ...
    s = scatter(x, NC,2,rColors{r}); hold on;
    %s.MarkerEdgeAlpha = 0.25;
    hline = refline([1 0]);
    hline.Color = 'k';
    hline.LineStyle = ':';
...
figure(f(1));
niceSave(results_path,sprintf('/%s_SubjFitsvsNC_%s', subjid, roi_name),[],[],{'png' 'svg'});
